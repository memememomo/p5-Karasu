...
subtest 'ping_reconnect' => sub {
    ...
    isnt($old_dbh, $db->dbh);
    is_deeply($db->single('mock_basic', { id => $pk }), {id => $pk, name => 'perl', delete_fg => 0});
};
...
subtest 'ping_reconnect_at_txn_begin' => sub {
    ...
    is_deeply($db->single('mock_basic', { id => $pk }), {id => $pk, name => 'python', delete_fg => 0});
};
...
subtest 'ping_reconnect_at_txn_scope' => sub {
    ...
    is_deeply($db->single('mock_basic', { id => $pk }), {id => $pk, name => 'ruby', delete_fg => 0});
};
